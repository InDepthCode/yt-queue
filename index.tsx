...
// Fetches title and thumbnail for a YouTube video or playlist
const fetchItemData = async (url: string): Promise<{ title: string; thumbnailUrl: string }> => {
    const playlistId = getYouTubePlaylistID(url);
    const videoId = getYouTubeID(url);

    // Handle pure playlist URLs using a dedicated public API
    if (playlistId && !videoId) {
        try {
            const response = await fetch(`https://api.youtubemultidownloader.com/playlist?url=${encodeURIComponent(url)}`);
            if (!response.ok) throw new Error('Playlist API response was not ok');
            ...
            console.error('Failed to fetch playlist data, falling back to placeholder. Error:', error);
            return { title: 'YouTube Playlist', thumbnailUrl: 'placeholder' };
        }
    }

    // Handle video URLs (including videos in playlists) using noembed.com
    try {
        const response = await fetch(`https://noembed.com/embed?url=${encodeURIComponent(url)}`);
        if (!response.ok) throw new Error('oEmbed network response was not ok');
        const data = await response.json();
        ...
        const currentVideoId = getYouTubeID(url);
        const thumbnailUrl = currentVideoId ? `https://img.youtube.com/vi/${currentVideoId}/mqdefault.jpg` : data.thumbnail_url;

        return { title: data.title || 'Untitled', thumbnailUrl };
    } catch (error) {
        console.error('Failed to fetch video data, falling back. Error:', error);
        return { title: 'Untitled', thumbnailUrl: '' };
    }
};
...
function App() {
  ...
  const handleSaveItem = async () => {
    ...
    const { title, thumbnailUrl } = await fetchItemData(newUrl);
